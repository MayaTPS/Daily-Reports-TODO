...
def get_arc(c):
    ...
    # --- Archive tab: rows 23-30 (spreadsheet rows) = array indices 20-27 after [2:] slice ---
    try: ws=sp.worksheet(ARC_TAB)
            except: ws=sp.worksheets()[6]
    all_rows=ws.get_all_values()[2:]  # skip 2 header rows; index 0 = spreadsheet row 3
    ...
    for idx,r in enumerate(all_rows):
                if not any(r): continue
                            t=g(r,A_TASK)
        if not t: continue
                    # Try Date Completed (col K, index 10) first, then fall back to Date (col B, index 1)
        date_val=g(r,A_DATE_DONE) or g(r,A_DATE)
        it={"p":g(r,A_PROP),"d":mkd(t,g(r,A_NOTE)),"a":g(r,A_ASGN),"s":g(r,A_STAT),"dt":date_val}
        pool.append(it)
        # Rows 23-30 in spreadsheet = indices 20-27 in all_rows (after [2:] skip)
        if 20<=idx<=27:
                        recent.append(it)
                        continue
                    # Also include any row with a date within the last 7 days
                    if date_val:
                                    for fmt in ("%m/%d/%Y","%Y-%m-%d","%d/%m/%Y","%m/%d/%y"):
                                                        try:
                                                                                if datetime.strptime(date_val,fmt)>=cut:
                                                                                                            ...
                                                                                                            # Also pull Done items from Operations Log as completed wins
                                            try:
    ...
    recent.sort(key=lambda x:x.get("dt",""),reverse=True)
    # Deduplicate by task description
    seen=set(); deduped=[]
    for it in recent:
                key=(it["p"],it["d"])
        if key not in seen: seen.add(key); deduped.append(it)
                # Pad with other archive items if under 4
    if len(deduped)<4:
                for x in pool:
                                key=(x["p"],x["d"])
...
def li(it):
        p=esc(it["p"]); d=esc(it["d"]); a=esc(it.get("a","")); s=esc(it.get("s","Done"))
    label=f"{p} - {d}" if p and p.lower()!="general" else d
    assignee=f" <em>({a})</em>" if a else ""
